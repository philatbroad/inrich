...
std::string CRandom::rand_letters (int n) {
    const char* a_to_z = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890";

    std::string str1 = "";
    for(int i=0; i<n; i++) {
        str1 = str1 + a_to_z[rand(62)];
    }
    return str1;
}
...
long unsigned int CRandom::rand (long unsigned int n)
{
    long unsigned int r = (long unsigned int)(rand() * n);
    if (r == n) r--;
    return r;
}
